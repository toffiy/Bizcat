...
class CustomerController {
  ...
  return _firestore
    ...
    for (var doc in snapshot.docs) {
      ...
      // ✅ Use buyerId as the key instead of email
      if (!buyersMap.containsKey(buyerId)) {
        buyersMap[buyerId] = CustomerWithStats(
          customer: Customer(
            id: buyerId,              // <-- changed here
            firstName: firstName,
            lastName: lastName,
            email: email,
            address: address,
      ...
      final buyerStats = buyersMap[buyerId]!;
...
Future<void> reportBuyer({
  required String id,            // buyerId from orders
  required String email,         // buyerEmail from orders
  required String name,          // buyerName from orders
  ...
  // 🔹 Save report under the buyer’s document
  await _firestore
      .collection("buyers")
      .doc(id)                     // ✅ use id from orders
      .collection("reports")
      .add({
    "reporterId": sellerId,
    "id": id,                      // ✅ buyerId stored as "id"
    "email": email,                // ✅ buyerEmail stored as "email"
    "name": name,                  // ✅ buyerName stored as "name"
    "reason": reason,
